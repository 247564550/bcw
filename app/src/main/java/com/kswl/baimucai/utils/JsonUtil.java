...
public class JsonUtil {
    ...
    private static Map<Class<?>, ArrayList<fieldEntity>> method_map = new HashMap<Class<?>,
            ArrayList<fieldEntity>>();
            ...
            if (object instanceof JSONArray) {
                ...
                if (root.length() > 0) {
                    ...
                    while (rootName.hasNext()) {
                        ...
                        if (!isHas) {
                            Log.d("JSON", "字段" + name + "在实体类" + entity + "不存在");
                        }
                    }
                } else {
                    Log.e("JSON", "数据长度不对 解析出错");
                }
                return entity;
            } else {
                return entity;
